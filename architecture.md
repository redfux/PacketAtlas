...
**Zeitleiste** (`timeline-view.js`): viertes Tab, ein Gantt-Diagramm derselben Verbindungen (Geräte-Paar + Protokoll + Port). Anders als bei Matrix/Graph/Verbindungen ist hier nicht die Geräteliste die Zeilenachse, sondern jede Verbindung selbst eine Zeile mit einem waagerechten Balken von `firstSeen` bis `lastSeen`; Zeilen sind nach Geräte-Paar gruppiert (`pairKey()`) und innerhalb eines Paars chronologisch sortiert, statt nach Volumen wie im Verbindungen-Tab. Da eine einzelne Verbindung nur Anfang/Ende kennt (keine Einzelpaket-Zeitstempel dazwischen, siehe Backlog-Hinweis in [features.md](features.md#backlog-nicht-bestandteil-dieser-version)), ist der Balken eine reine Aktivitätsspanne, keine Aussage über die tatsächliche Paketverteilung innerhalb dieser Spanne. Sehr kurze (Einzelpaket-)Verbindungen erhalten eine Mindestbreite, damit sie sichtbar bleiben.
