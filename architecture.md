...
### Port-Zuordnung in Tooltips und Kacheln

Ein **Geräte-Paar** aggregiert Datenverkehr in beide Richtungen – „Quell-Port" und „Ziel-Port" sind auf dieser Ebene nicht fest definiert, da sich die Richtung von Paket zu Paket ändern kann (mal sendet Gerät A, mal Gerät B). Statt eines einzigen, richtungsunabhängig vermischten `ports`-Sets hält `parser.worker.js` deshalb pro Paar zwei getrennte Sets: `portsA` (alle Ports, die Gerät A jemals genutzt hat, unabhängig von Sende-/Empfangsrichtung) und `portsB` (entsprechend für Gerät B). Tooltips/Kacheln zeigen dies als „Ports bei ⟨Gerät A⟩" / „Ports bei ⟨Gerät B⟩".

Eine einzelne **Connection** (Geräte-Paar + Protokoll + Service-Port) repräsentiert dagegen einen weitgehend richtungsstabilen Fluss, da die Service-Port-Gruppierung ohnehin beide Richtungen zusammenführt. Hier ist ein echtes „Quell-Port/Ziel-Port"-Paar sinnvoll und wird direkt vom ersten Paket übernommen, das diese Connection eröffnet hat (`connection.srcPort`/`connection.dstPort` in `parser.worker.js`, zusätzlich zum gruppierenden `connection.port`).
...
Der Export-Menüpunkt „Nur angeheftete Auswahl (.xlsx)" (deaktiviert, solange keine Kachel angeheftet ist) exportiert ausschließlich die aktuell angehefteten Elemente als eigenes Sheet „Auswahl" (`exportSelectionToExcel()` in `export-excel.js`), mit einheitlichen Spalten über alle drei Kachel-Typen hinweg (Typ, Gerät A/B, Protokoll(e), Quell-/Ziel-Port bzw. geräteweise Ports, Pakete, Bytes, Zeitraum) – unabhängig von der aktuellen Tab-/Filteransicht.
