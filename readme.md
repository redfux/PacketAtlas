...
2. **Ansicht wählen:** Über die Tabs oben zwischen **Matrix** (Adjazenzmatrix), **Graph** (Force-Directed-Netzwerkgraph) und **Verbindungen** (korrelierte Übersicht, welche Geräte über welche TCP/UDP-Ports miteinander kommunizieren) wechseln. Alle drei Ansichten zeigen dieselbe gefilterte Geräteauswahl. Im Verbindungen-Tab erhält jedes ausgewählte Gerät eine senkrechte Lebenslinie, jede eindeutige Kombination aus Geräte-Paar/Protokoll/Port einen beschrifteten Pfeil mit Paketanzahl (z. B. „TCP 443 — 25×"), sortiert nach Paketanzahl absteigend; bei sehr vielen unterschiedlichen Verbindungen werden nur die größten 400 gezeichnet (Warnhinweis mit Empfehlung, weiter zu filtern). Ein zeitlich chronologischer Ablauf je Einzelpaket ist bewusst nicht enthalten (siehe [features.md](features.md#backlog-nicht-bestandteil-dieser-version)).
...
4. **Metrik wählen:** Umschalter „Pakete / Bytes" bestimmt, welche Größe für Zellfarbe bzw. Kantenstärke verwendet wird (nur für Matrix/Graph – die Verbindungen-Ansicht zeigt immer die Paketanzahl je Verbindung und blendet diesen Umschalter aus).
...
6. **Details ansehen:** Beim Überfahren einer Matrixzelle, einer Kante/eines Knotens im Graphen bzw. eines Pfeils in der Verbindungen-Ansicht erscheint ein Tooltip mit Protokoll, Port, Paket-/Bytezahl und Zeitraum der Kommunikation.
7. **Exportieren:**
