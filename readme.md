...
2. **Ansicht wählen:** Über die Tabs oben zwischen **Matrix** (Adjazenzmatrix), **Graph** (Force-Directed-Netzwerkgraph) und **Sequenz** (Ablauf-/Sequenzdiagramm je Einzelpaket, wie bei Wiresharks „Flow Graph") wechseln. Alle drei Ansichten zeigen dieselbe gefilterte Geräteauswahl. Im Sequenz-Tab erhält jedes ausgewählte Gerät eine senkrechte Lebenslinie, jedes Paket einen chronologisch angeordneten, beschrifteten Pfeil zwischen den beteiligten Geräten; bei sehr vielen Paketen werden nur die ersten 400 gezeichnet (Warnhinweis mit Empfehlung, weiter zu filtern).
...
4. **Metrik wählen:** Umschalter „Pakete / Bytes" bestimmt, welche Größe für Zellfarbe bzw. Kantenstärke verwendet wird (nur für Matrix/Graph – die Sequenz-Ansicht arbeitet auf Einzelpaket-Ebene und blendet diesen Umschalter aus).
...
6. **Details ansehen:** Beim Überfahren einer Matrixzelle, einer Kante/eines Knotens im Graphen bzw. eines Pfeils in der Sequenz-Ansicht erscheint ein Tooltip mit Protokollen, Ports, Paket-/Bytezahl und Zeitpunkt/Zeitraum der Kommunikation.
7. **Exportieren:**
